...
if (opt.gpu == 1) then -- GPU run
  ...
  output = output:cuda()
  cmdstring="nvidia-smi -i 0 --query-gpu=power.limit,power.draw,utilization.gpu,utilization.memory --format=csv,nounits --loop-ms=500> ./nmt_l3_gpuPow_%d.txt &"% (batchsize)
  os.execute(cmdstring)
--  os.execute('nvidia-smi -i 0 --query-gpu=power.limit,power.draw,utilization.gpu,utilization.memory --format=csv,nounits --loop-ms=500> ./nmt_l3__gpuPow_opt.batch.txt &')
  -- measure gpu time
  gputime0 = sys.clock()
  run_dnn()
  gputime1 = sys.clock()
