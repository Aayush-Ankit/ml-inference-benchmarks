...
if (opt.gpu == 1) then -- GPU run
  ...
  output = output:cuda()
  cmdstring="nvidia-smi -i 0 --query-gpu=power.limit,power.draw,utilization.gpu,utilization.memory,memory.total,memory.used,memory.free --format=csv,nounits --loop-ms=500> ./nmt_l3_gpuPow_batchsize_%d.txt &"% (batchsize)
