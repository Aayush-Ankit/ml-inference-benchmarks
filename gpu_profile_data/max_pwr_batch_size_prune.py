...
filearg = sys.argv[1]
filename_list = list()
print "batchsize,util_gpu_pwr,total_gpu_pwr,gpu_util,gpu_memory_size,gpu_used_memory_gpu_free_memory"
		...
		for line in fp:
			gpu_query = line.split(",")
	                total_gpu_pwr_util.append(int(float(gpu_query[1])))
                        ...
                        memory_size.append(int(gpu_query[3]))
                        gpu_memory_used.append(int(gpu_query[4]))
                        gpu_memory_free.append(int(gpu_query[5]))
                else:
		        ...
		        total_gpu_pwr_util.sort(key=int,reverse=True)
                        ...
                        gpu_util.sort(key=int)
                        memory_size.sort(key=int)
                        gpu_memory_used.sort(key=int)
                        gpu_memory_free.sort(key=int)
